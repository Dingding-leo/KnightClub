...
The first board paint is deliberately quiet: Stockfish and the KnightBot fallback are created only for a real bot move or an explicit **Verify engine** action. A matching opening cue needs neither engine. Named-profile midgames request two PVs through one existing single-threaded, node-bounded search—not a second `go` command—and keep the same Easy 60 ms/6k nodes, Balanced 120 ms/18k and Strong 200 ms/45k caps. A short cancellable display floor keeps lightweight searches from looking unnaturally instant without consuming extra engine time. Fresh games show their full configuration, then collapse it to a compact opponent/side/time summary after the first ply so draw, resign and notation stay within reach. Custom-time fields retain their draft locally until **Use custom time**, so normal typing does not re-render the whole game shell and restored custom values remain editable. Live clock repainting is isolated from the Play shell, and memoized square wrappers keep selection, drag, premove and keyboard-focus changes from rebuilding every board button. Review's read-only board and long notation list are likewise isolated from progress-only full-review updates. Review, Train and Insights load only when opened, with hover/focus prefetch and a clear local reload recovery if an async workspace asset is unavailable. Review's ambient candidate line starts as Quick/one-line work, and a live bot move takes priority over optional review analysis. On desktop, contiguous not-yet-started active-session snapshots coalesce to the latest state without crossing ordinary persistence or clear-session boundaries. Persistent browser and desktop engines reuse only acknowledged unchanged UCI option blocks while retaining a readiness fence for every search, avoiding needless Hash resets during play and review; a cancelled queued native Play request is checked again after it obtains the engine lock, so it cannot perform needless setup or search work.
