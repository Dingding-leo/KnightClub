...
The first board paint is deliberately quiet: Stockfish and the KnightBot fallback are created only for a real bot move or an explicit **Verify engine** action. A matching opening cue needs neither engine. Named-profile midgames request two PVs through one existing single-threaded, node-bounded search—not a second `go` command—and keep the same Easy 80 ms/10k nodes, Balanced 160 ms/30k and Strong 280 ms/70k caps. A short cancellable display floor keeps lightweight searches from looking unnaturally instant without consuming extra engine time. Review, Train and Insights load only when opened, with hover/focus prefetch and a clear local reload recovery if an async workspace asset is unavailable. Review's ambient candidate line starts as Quick/one-line work, and a live bot move takes priority over optional review analysis. Persistent browser and desktop engines reuse only acknowledged unchanged UCI option blocks while retaining a readiness fence for every search, avoiding needless Hash resets during play and review.
