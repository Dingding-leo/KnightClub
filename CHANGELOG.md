...
- Live bot replies now use a short, cancellable 140/180/220 ms Easy/Balanced/Strong visual confirmation instead of 260/360/480 ms. Local opening cues and forced replies feel prompt while retaining a nonzero premove/feedback window, with no change to Stockfish's already-low search budget.
- A completed full-game Review now paints its scorecards before a dedicated serial local Worker clones, bounds and proves the report against its canonical PGN. The Worker returns an opaque prepared-save capability for browser/SQLite adapters, so long saves never replay chess on React's interaction thread; malformed results fail closed and no UI-thread fallback is allowed.
- Saved full-game Review restoration now uses a short-lived, latest-wins local Worker for browser mirrors and SQLite payloads. It strictly validates the complete report and its PGN away from the interaction thread, discards stale/cancelled work, returns the canonical timeline for Train → Review without a second replay, and reports an actionable error rather than falling back to a long UI-thread parse when Workers are unavailable.
- Full-game Review now publishes a usable, clearly provisional in-memory checkpoint after each fully classified non-final move. Players can inspect feedback while analysis continues, **Stop & keep** completed work, **Resume review** without repeating the next-position MultiPV baseline, or **Discard** it. Incomplete checkpoints are never saved or sent to Training, are cleared on source/reload boundaries, retain the original bounded settings and reject a changed engine identity; pausing also keeps ambient candidate analysis asleep so it returns local CPU.
- Default Rowan play now handles the standard `1. e4 e5 2. Nf3 Nc6` exchange through real local opening cues, avoiding the first 7 MB browser Stockfish startup on that common route; leaving Play or finishing a game also asks the desktop shared pool to release an actually idle native engine, while phone board status keeps the actionable SAN/premove cue visible without removing its full accessible announcement
...
- Opening an oversized selected Library game now commits a cancellable **Opening saved game** Play shell before its PGN is replayed in a separate verified Worker request. The previous board and clock stay paused until adoption; cancelling, leaving Play, a newer request or an invalid/late result preserves that board, while short PGNs retain their immediate open path. The worker also prepares the canonical review key with the chess snapshot, avoiding a second long main-thread hash pass
- Full-game Review now fixes its long-PGN Worker-routing decision at mount and keeps its visual progress continuous while reducing screen-reader updates to meaningful five-percent milestones
