...
describe('browser Stockfish UCI parsing', () => {
  ...
  it('matches desktop strength profiles while capping browser memory', () => {
    expect(resolveBrowserPlayOptions('easy')).toMatchObject({ elo: 1320, skillLevel: 2, moveTimeMs: 50, nodes: 4_000, hashMb: 16 })
    expect(resolveBrowserPlayOptions('strong')).toMatchObject({ elo: 2200, skillLevel: 14, moveTimeMs: 160, nodes: 24_000, hashMb: 32 })
...
describe('BrowserStockfishEngine', () => {
  it('handshakes, configures and searches in an isolated worker', async () => {
    ...
    expect(worker.messages).toContain('go movetime 100 nodes 10000')
