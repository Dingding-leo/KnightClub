...
Rowan Pike is the default opponent. Its conventional `1. e4 e5 2. Nf3 Nc6` replies are authored local cues, so the common first browser exchange does not download, compile or search the optional Stockfish WebAssembly runtime. A deviation from that exact legal history immediately falls back to the ordinary bounded Stockfish request.
...
Once a live game has safely finished or the player has left Play, the same idle predicate releases the retained Play runtime. Browser disposal drops its Stockfish and KnightBot Workers; desktop calls `stockfish_release_idle`, whose shared-pool `try_lock` makes the request a no-op while a Play, Verify or Review task owns the native process. This releases Stockfish’s NNUE and Hash memory without trading correctness for an unsafe mid-search shutdown.
...
| Balanced | 1700 | 8 | 50 ms | 1,500 | 1 | 16 MB |
| Strong | 2200 | 14 | 60 ms | 3,000 | 1 | 16 MB |

All three enable `UCI_LimitStrength` and stop on whichever time or node limit arrives first. The UI separately uses a short cancellable display floor (140/180/220 ms) so lower compute budgets and instant local opening cues remain perceptible without feeling held. The 1k/1.5k/3k caps reduce the old Balanced and Strong node ceilings by 50% and about 57% while preserving the one-thread, three-level strength ordering. The TypeScript browser/native adapters and the Rust `stockfish_best_move` command independently apply the same guard to Elo and Custom Play requests, so an old preference, direct renderer call or desktop payload cannot turn an ordinary bot reply into a 30-second, multi-threaded or multi-gigabyte search. These are initial product presets, not rating guarantees; future calibration should use a fixed test suite and recorded hardware.
