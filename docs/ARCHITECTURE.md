...
Native Play validates cancellation both before UCI setup and again after waiting for the single-engine mutex. A request cancelled while queued therefore cannot create/reset a supervisor, reapply options, send `position` or issue `go`; the same cancellation semantics already protect native analysis work.
