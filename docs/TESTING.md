...
**Latest fast historical-preview evidence (2026-07-23):** lint and typecheck passed; `npm test` passed with 55 files / 278 tests; `cargo test --manifest-path src-tauri/Cargo.toml` passed with 36 tests; production web and macOS Tauri builds passed; and the local Vite endpoint returned HTTP 200. Chess-domain contracts prove a later-half preview can clone the verified live game and undo only the short suffix without invoking replay, while a mismatched optional source falls through to the exact prefix replay and never alters the live history. The production entry measured 371.49 kB / 115.61 kB gzip and the lazy Review chunk 46.01 kB / 14.89 kB gzip. This is automated/build evidence, not a manual long-game browser or packaged-desktop walkthrough.
