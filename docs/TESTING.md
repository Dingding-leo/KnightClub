...
**Latest profile-candidate evidence (2026-07-23):** lint and typecheck passed; `npm test` passed with 46 files / 220 tests; `cargo test --manifest-path src-tauri/Cargo.toml` passed with 32 tests; production web and macOS Tauri builds passed; the local Vite endpoint returned HTTP 200. The fake UCI contracts prove that profile candidates use one unchanged `go movetime 160 nodes 30000`, one thread and `MultiPV 2`; malformed telemetry falls back to the valid `bestmove`. A manual in-app/profile walkthrough remains a release check.
