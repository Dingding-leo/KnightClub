...
5. Open a resignation or replacement confirmation, then a promotion chooser. In each overlay, press `N`, `U`, `F`, `⌘/Ctrl+Z` and confirm the paused board does not change; `Escape` must only close that overlay and restore the same game.
