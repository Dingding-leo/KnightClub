...
5. Open a resignation or replacement confirmation. Press `N`, `U`, `F`, `⌘/Ctrl+Z` and confirm the paused board does not change; `Escape` must only close the dialog and restore the same game.
