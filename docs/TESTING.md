...
2. While previewing, confirm board movement, Undo, Offer draw and Resign are unavailable; a queued premove is not rendered on the historical board but is still present after returning live. Escape and **Return to live** must restore the exact current board without changing clocks, the bot request or the saved session.
3. Let a bot move arrive while viewing an earlier ply. Confirm the preview stays on the selected move and the move list does not auto-scroll; return live and confirm the newly appended move and current board appear.
4. Repeat from a non-standard FEN start and after opening a saved game. Replacing/restarting/hydrating a game must return the view to live rather than reuse an old preview ply.
